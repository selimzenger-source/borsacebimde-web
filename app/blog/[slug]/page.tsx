...
// Build aninda API'den tum dinamik blog slug'larini alip statik sayfa uret.
// Statik export ile uyumlu: generateStaticParams + dynamicParams=false.
export async function generateStaticParams() {
  const blogs = await fetchAllBlogs();
  // Static klasoru zaten olan slug'lari hariç tut (cakismayi onlemek icin)
  ...
  return blogs
    .filter((b) => b.slug && !staticSlugs.has(b.slug))
    .map((b) => ({ slug: b.slug }));
}

// Static export: listede olmayan slug'lar 404 verir
    ...
    <article className="flex flex-col gap-6">
      ...
      <header className="flex flex-col gap-2">
        ...
        <div className="text-sm" style={{ color: 'var(--text-muted)' }}>
          ...
          {blog.category && (
            <span className="ml-2 px-2 py-0.5 rounded text-xs" style={{ backgroundColor: 'var(--bg-secondary)' }}>
              {blog.category}
            </span>
          )}
        </div>
